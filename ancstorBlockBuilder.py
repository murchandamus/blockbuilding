...
class BlockbuilderByAnces():
    ...
    def OrderTxs(self, txs):
        for tx in txs:
            ...
            self.mempool.txs[tx].feerate = self.mempool.txs[tx].ancestor_fee / self.mempool.txs[tx].ancestor_weight
        ...
        return OrderedDict(sorted(txs.items(), key=lambda tx: tx[1].feerate, reverse=True))
    ...
    def buildBlockTemplat(self, weightlimit):
        ...
        while txs:
            ...
            # Resort list
            txs = OrderedDict(sorted(txs.items(), key=lambda tx: tx[1].feerate, reverse=True))
