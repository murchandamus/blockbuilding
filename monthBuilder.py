            ...
            if file.endswith(blockId+'.mempool'):
                ...
                for k in blockMempool.txs.keys():
                    if k in self.globalMempool.txs:
                        blockMempool.txs[k].unconfirmed_ancestors = set(self.globalMempool.txs[k].unconfirmed_ancestors) | set(blockMempool.txs[k].unconfirmed_ancestors)
                        blockMempool.txs[k].immutable_parents = set(self.globalMempool.txs[k].immutable_parents) | set(blockMempool.txs[k].immutable_parents)
                        blockMempool.txs[k].children = set(self.globalMempool.txs[k].children) | set(blockMempool.txs[k].children)
